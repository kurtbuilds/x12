...
impl MeasurementMethodOrDevice {
    pub fn code(&self) -> &str {
        {
            use MeasurementMethodOrDevice::*;
            match self {
                BellowsMeter => "BM",
    ...
    pub fn from_code(code: &[u8]) -> Option<MeasurementMethodOrDevice> {
        use MeasurementMethodOrDevice::*;
        match code {
    ...
    fn from_description(description: &str) -> Option<MeasurementMethodOrDevice> {
        {
            use MeasurementMethodOrDevice::*;
            match description {
...
impl Serialize for MeasurementMethodOrDevice {
...
impl<'de> de::Visitor<'de> for Visitor {
    type Value = MeasurementMethodOrDevice;
    fn expecting(&self, formatter: &mut fmt::Formatter) -> fmt::Result {
        formatter.write_str("Measurement Method or Device")
    }
    fn visit_str<E>(self, v: &str) -> Result<Self::Value, E>
    where
        E: de::Error,
    {
        MeasurementMethodOrDevice::from_description(v)
            .ok_or_else(|| E::custom(
                format!("Invalid Measurement Method or Device: {}", v),
            ))
    }
    ...
    where
        E: de::Error,
    {
        MeasurementMethodOrDevice::from_code(v)
            .ok_or_else(|| E::custom(
                format!(
                    "Invalid Measurement Method or Device: {}", std::str::from_utf8(v)
                    .unwrap()
                ),
            ))
    }
}
impl<'de> Deserialize<'de> for MeasurementMethodOrDevice {
